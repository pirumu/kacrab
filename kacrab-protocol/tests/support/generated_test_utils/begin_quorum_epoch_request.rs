...
use kacrab_protocol::{generated::begin_quorum_epoch_request::*, *};

use crate::TestInstance;

impl TestInstance for BeginQuorumEpochRequestData {
    fn test_populated(version: i16) -> Self {
        Self {
            cluster_id: Some(KafkaString::from("test".to_owned())),
            voter_id: if version >= 1 { 12345_i32 } else { -1i32 },
            topics: vec![<TopicData as TestInstance>::test_populated(version)],
            leader_endpoints: if version >= 1 {
                vec![<LeaderEndpoint as TestInstance>::test_populated(version)]
            } else {
                Vec::new()
            },
    ...
    fn test_null_optionals(version: i16) -> Self {
        Self {
            cluster_id: None,
            voter_id: if version >= 1 { 0_i32 } else { -1i32 },
            topics: vec![<TopicData as TestInstance>::test_null_optionals(version)],
            leader_endpoints: if version >= 1 {
                vec![<LeaderEndpoint as TestInstance>::test_null_optionals(
                    version,
                )]
            } else {
                Vec::new()
    ...
    fn test_multi_element_collections(version: i16) -> Self {
        Self {
            ...
            voter_id: if version >= 1 { 23456_i32 } else { -1i32 },
            topics: vec![
            ...
            leader_endpoints: if version >= 1 {
                vec![
                    <LeaderEndpoint as TestInstance>::test_populated(version),
                    <LeaderEndpoint as TestInstance>::test_multi_element_collections(version),
                ]
            } else {
                Vec::new()
            },
    ...
    fn test_numeric_boundaries(version: i16) -> Self {
        Self {
            ...
            voter_id: if version >= 1 { i32::MIN } else { -1i32 },
            topics: vec![<TopicData as TestInstance>::test_numeric_boundaries(
                version,
            )],
            leader_endpoints: if version >= 1 {
                vec![<LeaderEndpoint as TestInstance>::test_numeric_boundaries(
                    version,
                )]
            } else {
                Vec::new()
    ...
    fn test_tagged_fields(version: i16) -> Self {
        Self {
            ...
            leader_endpoints: if version >= 1 {
                vec![<LeaderEndpoint as TestInstance>::test_tagged_fields(
                    version,
                )]
            } else {
                Vec::new()
...
fn encode_populated(version: i16) -> crate::MatrixResult<String> {
    let message = <BeginQuorumEpochRequestData as TestInstance>::test_populated(version);
...
fn encode_null_optionals(version: i16) -> crate::MatrixResult<String> {
    let message = <BeginQuorumEpochRequestData as TestInstance>::test_null_optionals(version);
    let mut out = BytesMut::new();
    message.write(&mut out, version)?;
    Ok(crate::hex(out.as_ref())?)
}
...
fn encode_empty_collections(version: i16) -> crate::MatrixResult<String> {
    let message = <BeginQuorumEpochRequestData as TestInstance>::test_empty_collections(version);
    let mut out = BytesMut::new();
    message.write(&mut out, version)?;
    Ok(crate::hex(out.as_ref())?)
}
...
fn encode_multi_element_collections(version: i16) -> crate::MatrixResult<String> {
    let message =
        <BeginQuorumEpochRequestData as TestInstance>::test_multi_element_collections(version);
    let mut out = BytesMut::new();
    message.write(&mut out, version)?;
    Ok(crate::hex(out.as_ref())?)
}
...
fn encode_numeric_boundaries(version: i16) -> crate::MatrixResult<String> {
    let message = <BeginQuorumEpochRequestData as TestInstance>::test_numeric_boundaries(version);
    let mut out = BytesMut::new();
    message.write(&mut out, version)?;
    Ok(crate::hex(out.as_ref())?)
}
...
fn encode_tagged_fields(version: i16) -> crate::MatrixResult<String> {
    let message = <BeginQuorumEpochRequestData as TestInstance>::test_tagged_fields(version);
    let mut out = BytesMut::new();
    message.write(&mut out, version)?;
    Ok(crate::hex(out.as_ref())?)
}
...
fn reencode(version: i16, hex_input: &str) -> crate::MatrixResult<String> {
    ...
    let message = BeginQuorumEpochRequestData::read(&mut input, version)?;
...
const MATRIX_CASES: &[crate::MatrixCase] = &[
    crate::MatrixCase {
        schema_name: "BeginQuorumEpochRequest",
        java_class: "org.apache.kafka.common.message.BeginQuorumEpochRequestData",
        version: 0i16,
